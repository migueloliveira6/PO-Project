...
public class Menu_Diretor {
    ...
    public static void authdiretor(ListaProfessores profs, ListaCursos cursos, ListaAluno alunos, Consola consola) {
    ...
    if (dirAutenticado != null && dirAutenticado.isDiretorDeCurso()) {
        Curso cursoDoDiretor = encontrarCursoDoDiretor(cursos, dirAutenticado); // Erro em ficheiros carregar

        if (cursoDoDiretor != null) {
            System.out.println("Bem-vindo, Diretor de Curso do curso: " + cursoDoDiretor.getDesignacao());
            Menu_Diretor.menudir(profs, cursos, dirAutenticado, cursoDoDiretor, alunos, consola);
        } else {
    ...
    public static void menudir(ListaProfessores profs, ListaCursos cursos, Professor dir, Curso curso, ListaAluno listaAlunos, Consola consola){
                int opcaoDiretor = -1;
                while(opcaoDiretor != 0){
                    ...
                    System.out.println("**************************************************************************");
                    opcaoDiretor = consola.lerInteiro();

                    switch(opcaoDiretor){
                        case 1:
                            System.out.println("Alteração da designação do curso: ");
                            System.out.println("Introduza o novo nome:");
                            String nome = consola.lerString();
                            Menu_Diretor.alterarCursoinfoDiretor(curso, nome);
                            break;
                        case 2:
                            System.out.println("Lista de alunos por curso: ");
                            Map<Curso, Integer> numeroAlunosPorCurso = listaAlunos.getNumeroAlunosPorCurso();
                            if (numeroAlunosPorCurso != null){
                                for (Map.Entry<Curso, Integer> entrada : numeroAlunosPorCurso.entrySet()) {
                                    System.out.println("Curso: " + entrada.getKey().getDesignacao()+ ", Número de Alunos: " + entrada.getValue());
                                }
                            }else{
                                 System.err.println("Não existe nenhum aluno associado a nenhum curso!");
                             } 
                            break;
                        case 3:
                            ...
                            if (numeroProfsPorCurso != null){
                                for (Map.Entry<Curso, Integer> entrada : numeroProfsPorCurso.entrySet()) {
                                        System.out.println("Curso: " + entrada.getKey().getDesignacao()+ ", Número de Professores: " + entrada.getValue());
                                    }
                                }else{
    ...
    public static void alterarCursoinfoDiretor(Curso curso, String nome){
        curso.setDesignacao(nome);
